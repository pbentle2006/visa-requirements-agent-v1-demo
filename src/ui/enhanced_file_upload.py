...
def _show_demo_mode_selection() -> Dict[str, Any]:
    ...
    return _show_sample_document_selection()


def _show_sample_document_selection() -> Dict[str, Any]:
    """Show sample document selection - used by both demo and live API modes."""
    policy_options = {
...
def _show_live_mode_upload() -> Optional[Dict[str, Any]]:
    ...
    input_method = st.radio(
        "Select how to provide the document:",
        ["Use sample document", "Upload your own document"],
        horizontal=True
    )
    
    if input_method == "Use sample document":
        # Show sample document selection (same as demo mode)
        return _show_sample_document_selection()
    else:
        # File upload
